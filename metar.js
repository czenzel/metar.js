    ...
    METAR.prototype.parseRecentSignificantWeather = function() {
		...
		METAR.prototype.parseHourlyPrecipitation = function() {
			for (var i = 0; i < this.fields.length; i++) {
				...
				if (current.length === 5 && "P" === current[0]) {
					var timePrecip = current.substr(1);
					timePrecip = parseInt(timePrecip);
					timePrecip = timePrecip * 0.01;
					return timePrecip;
				}
			}
		};
		...
		METAR.prototype.parsePrecipitation = function() {
			this.result.precipitation = {
				...
				threeHours: this.parse3HourPrecipitation(),
				sixHours: this.parse6HourPrecipitation()
			};
		};
